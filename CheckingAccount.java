...
public class CheckingAccount extends Account {
    ...
    private static final double INTEREST_PER_ANNUM = 0.001d;
        ...
        if(!getTransactions().isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int d = 0; d < days; d++) {
                    currentBalance += currentBalance * (INTEREST_PER_ANNUM / (getTransactions().get(i).getDate().plusDays(d).isLeapYear() ? 366 : 365));  //check if the day we are calculating the earnings falls to a leap year or not
                }
    ...
    public List<Transaction> buildTransactions() {
        ...
        if(!getTransactions().isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                lastBalance = currentBalance;
                for (int d = 0; d < days; d++) {
                    newAmount = currentBalance * (INTEREST_PER_ANNUM / (getTransactions().get(i).getDate().plusDays(d).isLeapYear() ? 366 : 365));  //check if the day we are calculating the earnings falls to a leap year or not
