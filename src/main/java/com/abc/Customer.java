...
public class Customer {
    ...
    public Customer(String name) {
    	if(name == null) {
    		throw new IllegalArgumentException(Customer.class + 
    				"::name cannot be null.");
    	}
        this.name = name;
     ...
     * @param aName is the new name. If null it's ignored.
     */
     ...
     * @param account is the account the customer is opening. Ignored if null.
     ...
     * @return Returns a clone list of all the customers accounts. Account references remain the same.
     */
    ...
    /**
     * Get the total annual interest a customer is owed given their current holdings.
    ...
    /**
     * Transfer holdings from one account to another account of which the customer owns.
     * @param from is the account to take the funds from. The account must have enough funds and belong to the customer.
     * @param to is the account to add the funds to. The account must belong to the customer.
     * @param amount is the amount to transfer between accounts.
     ...
     * @param accounts is a array or singular account to check if the account owns.
    ...
    public boolean checkOwnAccounts(Account... accounts) {
    	...
    	if(accounts != null && accounts.length > 0) {
    		...
    		//If there is a non-null entry there must be an account not belonging to this customer.
    		for(Account a : accounts) {
    			if(a != null) {
    				result = false;
    				break;
    ...
    public String toString() {
    	return "Accounts: " + getNumberOfAccounts() + "  Total Holdings: " + Transaction.toCurrecy(getTotalAccountHoldings());
    }
}
