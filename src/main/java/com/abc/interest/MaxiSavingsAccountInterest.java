...
public class MaxiSavingsAccountInterest implements InterestScheme {
	...
	// Penalty interest earned when account has been withdrawn from in the last 10
	// days.
	...
	/**
	 * Get the interest for a given list of transactions. Interest is calculated in
	 * bands, the first 1000, the next 1000, and anything above that. If a
	 * withdrawal has been mad in the last 10 days then a constant penalty rate will
	 * be used.
	 * 
	 * @param transactions
	 *            is a list of transactions you want to calculate interest for.
	 ...
	 * 
	 * @param total
	 *            is the amount to apply interest to. If null zero is assumed.
	...
	private BigDecimal standardInterest(BigDecimal total) {
		...
		if (result.compareTo(THRESHOLD1) <= 0) {
			...
			if (result.compareTo(THRESHOLD2) <= 0) {
				...
				result = result.multiply(THRESHOLD1_INTEREST).add(BASE_INTEREST_MAX);
			} else { // Total is more than secondary threshold
				// Calculate all interest together
				result = result.subtract(THRESHOLD1);
				result = result.multiply(THRESHOLD2_INTEREST).add(THRESHOLD1_INTEREST_MAX).add(BASE_INTEREST_MAX);
			}
		}
	 ...
	 * 
	 * @param transactions
	 *            is the list of transactions to check.
	 * @return Returns true if there has been a transaction in the past 10 days,
	 *         else false.
	 */
	private boolean checkPenalty(List<Transaction> transactions) {
		...
		long penaltyPeriod = DateProvider.getInstance().now().getTime() - TimeUnit.DAYS.toMillis(PENALTY_DAYS);

		// Fore each transaction check they are not within the penalty period.
		for (Transaction t : transactions) {
			if (t.getAmount().compareTo(BigDecimal.ZERO) < 0 && t.getDate().getTime() >= penaltyPeriod) {
				result = true;
				break;
			}
		}
